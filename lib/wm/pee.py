...
The embed loop predicts from the already-modified carrier (canonical PEE, Thodi &
Rodriguez 2007). For LeftNeighbourPredictor this creates an implicit second-difference
recursion (D[i] ≈ bit − D[i-1], |root|=1), keeping errors small and BPS high.

Lagrange extrapolation is NOT compatible with this causal approach: its characteristic
equation has a root |r| ≈ 2.4 > 1 causing exponential divergence. Use HSEmbedder
with LagrangePredictor instead.
"""
...
class PEEEmbedder(WatermarkEmbedder):
    ...
    def __init__(
        ...
        *,
        ...
        if block_len != 1:
            raise InvalidConfig("PEE поддерживает только block_len=1")
        predictor = predictor if predictor is not None else LeftNeighbourPredictor()
        if isinstance(predictor, LagrangePredictor):
            raise InvalidConfig(
                "LagrangePredictor is incompatible with PEE: causal extrapolation "
                "from the modified carrier is numerically unstable (characteristic "
                "root |r| ≈ 2.4 > 1). Use HSEmbedder(predictor=LagrangePredictor(n)) instead."
            )
